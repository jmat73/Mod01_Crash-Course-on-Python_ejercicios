...
def calculate_frequencies(file_contents):
  ...
  # Prepare the string for count 
    for i in range(len(punctuations)):                   # Checking if the characters not valid for the count
        ...
        words= text.split(" ")                               # Prepare the string div in words
   
  
    for word in words:                                   # Checking if the characters or not, if true go to result
        if word.isalpha():
            result.append(word)
              
    
    for word_1 in uninteresting_words:                  # Checking if the characters in uninteresting_words, if true remove of result
    ...
    for word in result:                             # result is clean of uninteresting_words and the punctuations y go to create dict
        if word not in dicc_count:                  # Check if the word is in dictionary and  needs to be counted or not  
            dicc_count[word] = 1                   
        else:
    ...
    cloud.generate_from_frequencies(dicc_count)
